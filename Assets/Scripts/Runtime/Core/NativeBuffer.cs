...
namespace Unity.Collections
{
	...
	public unsafe struct NativeBuffer<T> : IDisposable
        where T : struct
	{
        ...
        public unsafe NativeBuffer(Allocator i_label) : this (1, i_label, 2) { }
	    public unsafe NativeBuffer(int capacity, Allocator i_label) : this (capacity, i_label, 2) { }
        ...
        // 区间数据拷贝，不改变buffer长度
        public void RangeSwapBackTargetIndex(int index, int length, int end_index)
        {
#if ENABLE_UNITY_COLLECTIONS_CHECKS
		    AtomicSafetyHandle.CheckWriteAndBumpSecondaryVersion(m_Safety);

            if( index < 0 || index >= Length  || end_index > Length || index > end_index)
                throw new ArgumentOutOfRangeException(index.ToString());
#endif
            int need_swap = end_index - index - length;
            if (need_swap > length)
                need_swap = length;
            ...
            {
                var sizeOf = UnsafeUtility.SizeOf<T>();
                int p_end_index = end_index - need_swap;
                UnsafeUtility.MemCpy((byte*)m_ListData->buffer + index * sizeOf,
                                                      (byte*)m_ListData->buffer + p_end_index* sizeOf, sizeOf * need_swap);
            }
        }
	    ...
	    public static implicit operator NativeArray<T> (NativeBuffer<T> nativeList)
	    {
    ...
    {
        NativeBuffer<T> m_Array;

        public NativeListDebugView(NativeBuffer<T> array)
        {
            m_Array = array;
        }
...
#if ENABLE_UNITY_COLLECTIONS_CHECKS
        public static AtomicSafetyHandle GetAtomicSafetyHandle<T>(ref NativeBuffer<T> nativeList) where T : struct
        {
            return nativeList.m_Safety;
        }
#endif

        public static unsafe void* GetInternalListDataPtrUnchecked<T>(ref NativeBuffer<T> nativeList) where T : struct
        {
